...
// this project's config (read + distinct write endpoint, commitment arg).
...
export function getSolanaConnection(
  ...
  if (opts.write) {
    return new Connection(config.rpcWriteUrl, commitment);
  }
  return new Connection(config.rpcReadUrl, commitment);
}
